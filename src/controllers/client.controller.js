...
export async function importClient(req, res) {
    try {
        const result = await clientService.importClient(req.body);
        return res.status(result.status).json(result.data);
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: 'Wystąpił błąd podczas importowania klienta.' });
    }
}